...
def run_2d_lmk_fitting(model_fname, template_fname, flame_lmk_path, texture_mapping, target_img_path, target_lmk_path, out_path):
    ...
    if target_lmk_path is None:
        from landmarks_utils import detect_landmarks
        lmk_2d = detect_landmarks(target_img)
    else:
        assert os.path.exists(target_lmk_path),\
            "Failed to find landmarks path {}".format(target_lmk_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument('target_img_path', help='Path of the target image')
    ...
    parser.add_argument('--target_lmk_path', default=None, help='2D landmark file that should be fitted (landmarks must be corresponding with the defined FLAME landmarks)')
    # Output path
    parser.add_argument('--out_path', default=None, help='Path of the fitting output')
    ...
    assert os.path.exists(args.target_img_path),\
        "Failed to find image {}".format(args.target_img_path)
    if args.out_path is None:
        args.out_path = "./results/{}".format(os.path.splitext(os.path.basename(args.target_img_path))[0])

    run_2d_lmk_fitting(
        args.model_fname, args.template_fname,
        args.flame_lmk_path, args.texture_mapping,
        args.target_img_path, args.target_lmk_path, args.out_path
    )
